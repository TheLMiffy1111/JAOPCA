...
public class AthanorSerializer implements IRecipeSerializer {
	...
	public AthanorSerializer(ResourceLocation key, Object input, int inputCount, Object tool, Object[] output, Map<SpiritusType, Double> spiritusCosts, boolean spiritusBoost) {
		this(key, input, inputCount, tool, null, 0, output, null, 0, spiritusCosts, spiritusBoost);
	}

	public AthanorSerializer(ResourceLocation key, Object input, int inputCount, Object tool, Object fluidInput, int fluidInputAmount, Object[] output, Object fluidOutput, int fluidOutputAmount, Map<SpiritusType, Double> spiritusCosts, boolean spiritusBoost) {
	...
	public JsonElement get() {
		...
		if(guaranteedOutputs.isEmpty() && chanceOutputs.isEmpty() && fluidResult.isEmpty()) {
			throw new IllegalArgumentException("Empty outputs in recipe "+key+": "+Arrays.deepToString(output)+", "+fluidOutput);
		}
		AthanorRecipe recipe = new AthanorRecipe(Optional.ofNullable(ingTool), List.of(ing), guaranteedOutputs, chanceOutputs, Optional.ofNullable(fluidIng), fluidResult.isEmpty() ? Optional.empty() : Optional.of(fluidResult), spiritusCosts, spiritusBoost);
