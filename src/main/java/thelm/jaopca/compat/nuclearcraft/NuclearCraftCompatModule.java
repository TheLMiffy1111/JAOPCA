...
public class NuclearCraftCompatModule implements IModule {
	...
	private static final Set<String> TO_DUST_BLACKLIST = new TreeSet<>(List.of(
			"aluminium", "aluminum", "baratol", "beryllium", "boron", "boron_arsenide", "boron_nitride", "bronze",
			"calcium", "carbon_manganese", "carobbite", "charcoal", "chromium", "coal", "cobalt", "copper", "diamond",
			"electrum", "emerald", "extreme", "ender_pearl", "ferroboron", "fluorite", "gold", "graphite", "hafnium",
			"hard_carbon", "hsla_steel", "iron", "lapis", "lead", "lead_platinum", "lithium", "lithium_manganese_dioxide",
			"magnesium", "manganese", "manganese_dioxide", "manganese_oxide", "netherite", "netherite_scrap", "niobium",
			"obsidian", "osmium", "palladium", "platinum", "potassium", "pyrolitic_carbon", "rhodochrosite", "shibuichi",
	...
	private static final Set<String> TO_PLATE_BLACKLIST = new TreeSet<>(List.of(
			...
			"manganese", "netherite", "palladium", "platinum", "pyrolitic_carbon", "sic_sic_cmc", "silver", "steel",
			"thermoconducting", "thorium", "tin", "tough_alloy", "uranium", "zinc", "zirconium"));
	private static final Set<String> TO_CRYSTAL_BLACKLIST = new TreeSet<>(List.of(
			"boron_nitride", "carobbiite", "diamond", "emerald", "fluorite", "lapis", "obsidian", "quartz", "rhodochrosite", "villiaumite"));
	private static final Set<String> MOLTEN_BLACKLIST = new TreeSet<>(List.of(
			"aluminium", "aluminum", "arsenic", "baratol", "barium_nitrate", "beryllium", "boron", "boron_arsenide",
			"bronze", "caesium_137", "carbon_manganese", "cobalt", "copper", "corium", "electrum", "enderium", "europium_155",
			"extreme", "ferroboron", "gold", "hard_carbon", "hsla_steel", "iron", "lapis", "lead", "lead_platinum",
			"lithium", "lithium_manganese_dioxide", "magnesium", "magnesium_diboride", "manganese", "manganese_dioxide",
			"manganese_oxide", "molybdenum", "obsidian", "palladium", "platinum", "polonium", "potassium_hydroxide",
			"potassium_iodide", "promethium_147", "purpur", "ruthenium_106", "shibuichi", "sic_sic_cmc", "silicon_carbide",
			"silver", "sodium_hydroxide", "steel", "strontium_90", "sulfur", "thermoconducting", "thorium", "tin", "tin_silver",
			"tough_alloy", "uranium", "zinc", "zircaloy", "zirconium_molybdenum", "zirconium"));
	...
	public void onCommonSetup(IModuleData moduleData, FMLCommonSetupEvent event) {
		...
		for(IMaterial material : moduleData.getMaterials()) {
			...
			if(!type.isDust() && !TO_DUST_BLACKLIST.contains(name) && !configToDustBlacklist.contains(name)) {
				...
				if(itemTags.contains(dustLocation)) {
					helper.registerManufactoryRecipe(
							miscHelper.getRecipeKey("nuclearcraft.material_to_dust", name),
							materialLocation, 1, dustLocation, 1, 200, 50);
				}
			}
			if(type.isIngot() && !TO_PLATE_BLACKLIST.contains(name) && !configToPlateBlacklist.contains(name)) {
				...
				if(itemTags.contains(plateLocation)) {
					helper.registerPressurizerRecipe(
							miscHelper.getRecipeKey("nuclearcraft.material_to_plate", name),
							materialLocation, 1, plateLocation, 1, 200, 50);
				}
			}
			if(type.isCrystalline() && !TO_CRYSTAL_BLACKLIST.contains(name) && !configToCrystalBlacklist.contains(name)) {
				...
				if(itemTags.contains(dustLocation)) {
					helper.registerPressurizerRecipe(
							miscHelper.getRecipeKey("nuclearcraft.dust_to_material", name),
							dustLocation, 1, materialLocation, 1, 200, 50);
				}
			}
			if(!type.isDust()) {
				...
				if(fluidTags.contains(moltenLocation)) {
					if(!MOLTEN_BLACKLIST.contains(name) && !configMaterialToMoltenBlacklist.contains(name)) {
						...
						helper.registerMelterRecipe(
								miscHelper.getRecipeKey("nuclearcraft.material_to_molten", name),
								materialLocation, 1, moltenLocation, baseAmount, 200, 50);
					}
					if(!MOLTEN_BLACKLIST.contains(name) && !configDustToMoltenBlacklist.contains(name)) {
						ResourceLocation dustLocation = miscHelper.getTagLocation("dusts", name);
						if(itemTags.contains(dustLocation)) {
							helper.registerMelterRecipe(
									miscHelper.getRecipeKey("nuclearcraft.dust_to_molten", name),
									dustLocation, 1, moltenLocation, baseAmount, 200, 50);
						}
					}
					if(!MOLTEN_BLACKLIST.contains(name) && !configToMaterialBlacklist.contains(name)) {
						ResourceLocation materialLocation = miscHelper.getTagLocation(type.getFormName(), name);
						helper.registerIngotFormerRecipe(
								miscHelper.getRecipeKey("nuclearcraft.molten_to_material", name),
								moltenLocation, baseAmount, materialLocation, 1, 200, 50);
					}
				}
			}
		}
