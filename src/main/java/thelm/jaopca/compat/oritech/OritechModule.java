...
public class OritechModule implements IModule {
	...
	public void onCommonSetup(IModuleData moduleData, FMLCommonSetupEvent event) {
		...
		Fluid sheolFire = BuiltInRegistries.FLUID.getValue(Identifier.parse("oritech:still_sheol_fire"));
		Fluid sulfuricAcid = BuiltInRegistries.FLUID.getValue(Identifier.parse("oritech:still_sulfuric_acid"));
		Fluid mineralSlurry = BuiltInRegistries.FLUID.getValue(Identifier.parse("oritech:still_mineral_slurry"));
		Item fluxite = BuiltInRegistries.ITEM.getValue(Identifier.parse("oritech:fluxite"));
		for(IMaterial material : formRequest.getMaterials()) {
			IItemInfo clumpInfo = itemFormType.getMaterialFormInfo(clumpForm, material);
			Identifier clumpLocation = miscHelper.getTagLocation(commonClump ? "clumps" : "oritech:clumps", material.getName());
			IItemInfo smallClumpInfo = itemFormType.getMaterialFormInfo(smallClumpForm, material);
			Identifier smallClumpLocation = miscHelper.getTagLocation("oritech:small_clumps", material.getName());
			IItemInfo gemInfo = itemFormType.getMaterialFormInfo(gemForm, material);
			Identifier gemLocation = miscHelper.getTagLocation("oritech:gems", material.getName());
			...
			Identifier extraTinyDustLocation = miscHelper.getTagLocation("tiny_dusts", material.getExtra(1).getName());
			...
			Identifier createCrushedLocation = miscHelper.getTagLocation("create:crushed_raw_materials", material.getName());
		...
		for(IMaterial material : moduleData.getMaterials()) {
			...
			Identifier rawMaterialLocation = miscHelper.getTagLocation("raw_materials", material.getName());
			Identifier dustLocation = miscHelper.getTagLocation("dusts", material.getName());
			Identifier tinyDustLocation = miscHelper.getTagLocation("tiny_dusts", material.getName());
