	...
	public void onCommonSetup(IModuleData moduleData, FMLCommonSetupEvent event) {
		...
		for(IMaterial material : moduleData.getMaterials()) {
			...
			helper.registerMineralGrinderRecipe(
					miscHelper.getRecipeKey("electrodynamics.ore_to_material_grinder", material.getName()),
					oreLocation, 1, materialLocation, isCrystal ? 3 : 2, isCrystal ? 1 : 0.2, 200, 350);
			if(!isCrystal) {
				helper.registerMineralCrusherRecipe(
						miscHelper.getRecipeKey("electrodynamics.ore_to_material_crusher", material.getName()),
