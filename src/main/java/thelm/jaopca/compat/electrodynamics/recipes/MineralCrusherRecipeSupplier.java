...
public class MineralCrusherRecipeSupplier implements Supplier<MineralCrusherRecipe> {
	...
	public MineralCrusherRecipeSupplier(ResourceLocation key, Object input, int inputCount, Object output, int outputCount, double experience, int time, double energy) {
		this(key, input, inputCount, output, outputCount, ItemStack.EMPTY, 0, 0, experience, time, energy);
	}

	public MineralCrusherRecipeSupplier(ResourceLocation key, Object input, int inputCount, Object output, int outputCount, Object secondOutput, int secondOutputCount, double secondChance, double experience, int time, double energy) {
	...
	public MineralCrusherRecipe get() {
		...
		return new MineralCrusherRecipe(key, Collections.singletonList(ing), stack, experience, time, energy, itembi, Collections.emptyList());
	}
}
