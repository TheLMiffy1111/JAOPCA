...
public class CrushingRecipeSerializer implements IRecipeSerializer {
	...
	public JsonElement get() {
		...
		firstJson.addProperty("chance", outputChance);
		firstJson.add("result", ItemStack.CODEC.encodeStart(JsonOps.INSTANCE, stack).get().orThrow());
		resultJson.add(firstJson);
		if(!secondStack.isEmpty()) {
			JsonObject secondJson = new JsonObject();
			secondJson.addProperty("chance", secondChance);
			secondJson.add("result", ItemStack.CODEC.encodeStart(JsonOps.INSTANCE, secondStack).get().orThrow());
			resultJson.add(secondJson);
		}
