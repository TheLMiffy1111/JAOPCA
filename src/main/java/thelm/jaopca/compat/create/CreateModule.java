...
public class CreateModule implements IModule {
	...
	public void onCommonSetup(IModuleData moduleData, FMLCommonSetupEvent event) {
		...
		ResourceLocation deepslateOreLocation = ResourceLocation.parse("forge:ores_in_ground/deepslate");
		ResourceLocation netherrackOreLocation = ResourceLocation.parse("forge:ores_in_ground/netherrack");
		ResourceLocation endstoneOreLocation = ResourceLocation.parse("forge:ores_in_ground/end_stone");
		Item xpNugget = BuiltInRegistries.ITEM.get(ResourceLocation.parse("create:experience_nugget"));
		...
		for(IMaterial material : crushedForm.getMaterials()) {
			...
			String configByproduct = config.getDefinedString("create.byproduct", "minecraft:cobblestone",
					s->BuiltInRegistries.ITEM.containsKey(ResourceLocation.parse(s)), "The default byproduct material to output in Create's crushing.");
			Item byproduct = BuiltInRegistries.ITEM.get(ResourceLocation.parse(configByproduct));

			helper.registerCrushingRecipe(
					miscHelper.getRecipeKey("create.default_ore_to_crushed", material.getName()),
			...
			helper.registerCrushingRecipe(
					miscHelper.getRecipeKey("create.deepslate_ore_to_crushed", material.getName()),
			...
			helper.registerCrushingRecipe(
					miscHelper.getRecipeKey("create.netherrack_ore_to_crushed", material.getName()),
					CompoundIngredientObject.intersection(new Object[] {
			...
			if(itemTags.contains(endstoneOreLocation)) {
				helper.registerCrushingRecipe(
						miscHelper.getRecipeKey("create.end_stone_ore_to_crushed", material.getName()),
						CompoundIngredientObject.intersection(new Object[] {
			...
			if(material.getType() == MaterialType.INGOT) {
				...
				helper.registerCrushingRecipe(
						miscHelper.getRecipeKey("create.raw_material_to_crushed", material.getName()),
				...
				if(itemTags.contains(rawStorageBlockLocation)) {
					helper.registerCrushingRecipe(
							miscHelper.getRecipeKey("create.raw_storage_block_to_crushed", material.getName()),
			...
			api.registerSmeltingRecipe(
					miscHelper.getRecipeKey("create.crushed_to_material_smelting", material.getName()),
					crushedLocation, materialLocation, 1, 0.1F, 200);
			api.registerBlastingRecipe(
					miscHelper.getRecipeKey("create.crushed_to_material_blasting", material.getName()),
