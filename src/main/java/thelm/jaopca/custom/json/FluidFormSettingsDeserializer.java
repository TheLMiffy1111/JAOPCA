...
public class FluidFormSettingsDeserializer implements JsonDeserializer<IFluidFormSettings> {
	...
	public IFluidFormSettings deserialize(JsonElement jsonElement, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
		...
		if(json.has("luminosity")) {
			...
			settings.setLuminosityFunction(helper.deserializeType(json, "luminosity", context, FormTypeHandler.INT_FUNCTION_TYPE));
		}
		if(json.has("density")) {
		...
		if(json.has("gaseous")) {
			...
			if(!functionJson.has("gaseous")) {
				functionJson.addProperty("gaseous", false);
			}
			settings.setIsGaseousFunction(helper.deserializeType(json, "gaseous", context, FormTypeHandler.PREDICATE_TYPE));
		}
		if(json.has("rarity")) {
