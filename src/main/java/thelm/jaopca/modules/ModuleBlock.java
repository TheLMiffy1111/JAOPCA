...
public class ModuleBlock extends ModuleBase {
	...
	public void init() {
		for(IOreEntry entry : JAOPCAApi.ENTRY_NAME_TO_ORES_MAP.get("block")) {
			...
			GameRegistry.addRecipe(new ShapelessOreRecipe(Utils.getOreStack("block", entry, 1), new Object[] {
					s+entry.getOreName(),
					s+entry.getOreName(),
					s+entry.getOreName(),
					s+entry.getOreName(),
			...
			GameRegistry.addRecipe(new ShapelessOreRecipe(Utils.getOreStack(s, entry, 9), new Object[] {
