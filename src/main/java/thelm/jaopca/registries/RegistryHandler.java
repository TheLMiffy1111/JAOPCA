...
public class RegistryHandler {
	...
	private static final TreeMap<Identifier, DeferredRegister<?>> DEFERRED_REGISTERS = new TreeMap<>();
	...
	public static <T, I extends T> DeferredHolder<T, I> registerRegistryEntry(ResourceKey<? extends Registry<T>> registry, String name, Supplier<I> entry) {
		...
		return ((DeferredRegister<T>)DEFERRED_REGISTERS.computeIfAbsent(registry.identifier(), _->{
	...
	public static <T, I extends T> DeferredHolder<T, I> registerRegistryEntry(Identifier registry, String name, Supplier<I> entry) {
		Objects.requireNonNull(entry);
		return ((DeferredRegister<T>)DEFERRED_REGISTERS.computeIfAbsent(registry, _->{
			DeferredRegister<?> register = DeferredRegister.create(registry, "jaopca");
			register.register(EVENT_BUS.get());
			return register;
		})).register(name, entry);
