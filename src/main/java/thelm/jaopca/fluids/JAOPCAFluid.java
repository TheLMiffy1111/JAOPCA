...
public class JAOPCAFluid extends Fluid implements IMaterialFormFluid {
	...
	public JAOPCAFluid(IForm form, IMaterial material, IFluidFormSettings settings) {
		...
		maxLevel = settings.getMaxLevelFunction().applyAsInt(material);
	...
	protected BlockState getBlockState(IFluidState fluidState) {
		IMaterialFormFluidBlock materialFormBlock = FluidFormType.INSTANCE.getMaterialFormInfo(form, material).getMaterialFormFluidBlock();
		...
		return materialFormBlock.asBlock().getDefaultState().with(blockLevelProperty, blockLevel);
	}
