	...
	public static void registerCustomTags() {
		...
		for(String line : ConfigHandler.CUSTOM_TAGS) {
			for(String entry : lineSplitter.split(line)) {
				...
				if(!ResourceLocation.isValidResourceLocation(registry)) {
				...
				if(!ResourceLocation.isValidResourceLocation(tagSplit.get(0))) {
					LOGGER.warn("Custom tag entry [{}] has invalid object", entry);
					continue;
				}
				if(!ResourceLocation.isValidResourceLocation(tagSplit.get(1))) {
					LOGGER.warn("Custom tag entry [{}] has invalid tag", entry);
					continue;
				}
				registerTag(ResourceKey.createRegistryKey(new ResourceLocation(registry)), new ResourceLocation(tagSplit.get(1)), ()->new ResourceLocation(tagSplit.get(0)));
			...
			Pack packInfo = Pack.create("inmemory:jaopca", true, ()->{
				...
				TAGS_INJECT.asMap().forEach((registry, map)->{
					...
					map.asMap().forEach((tagLocation, objLocations)->{
						...
						objLocations.stream().map(Supplier::get).filter(Objects::nonNull).distinct().forEach(l->builder.addOptionalElement(l, "inmemory:jaopca"));
						pack.putJson(PackType.SERVER_DATA, new ResourceLocation(tagLocation.getNamespace(), path+tagLocation.getPath()+".json"), builder.serializeToJson());
					});
				});
