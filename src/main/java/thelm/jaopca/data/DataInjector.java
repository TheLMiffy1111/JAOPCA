...
public class DataInjector extends ReloadListener<Object> {
	...
	public static boolean registerTag(String type, ResourceLocation tagLocation, Supplier<ResourceLocation> objLocation) {
		...
		return TAGS_INJECT.getUnchecked(type).put(tagLocation, objLocation);
	}
	...
	public static Set<ResourceLocation> getInjectTags(String type) {
		Objects.requireNonNull(type);
		return TAGS_INJECT.getUnchecked(type).keySet();
	}
	...
	public static void registerCustomTags() {
		...
		Splitter typeSplitter = Splitter.on('#').limit(2).trimResults();
		...
		for(String line : ConfigHandler.CUSTOM_TAGS) {
			for(String entry : lineSplitter.split(line)) {
				List<String> typeSplit = typeSplitter.splitToList(entry);
				String type = "items";
				String tag;
				if(typeSplit.size() == 2) {
					type = typeSplit.get(0);
					tag = typeSplit.get(1);
				}
				else {
					tag = typeSplit.get(0);
				...
				registerTag(type, new ResourceLocation(tagSplit.get(1)), ()->new ResourceLocation(tagSplit.get(0)));
	...
	public static class PackFinder implements IPackFinder {
		...
		public void loadPacks(Consumer<ResourcePackInfo> packList, ResourcePackInfo.IFactory factory) {
			ResourcePackInfo packInfo = ResourcePackInfo.create("inmemory:jaopca", true, ()->{
				...
				TAGS_INJECT.asMap().forEach((type, map)->{
