...
/**
 * Modification of Forge's fluid model that asks for textures instead of fluids
...
public final class ModelFluidTextured implements IRetexturableModel {
	...
	public IBakedModel bake(IModelState state, VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter) {
		...
		return new BakedFluidTextured(state.apply(Optional.<IModelPart>absent()), map, format, bakedTextureGetter.apply(still), bakedTextureGetter.apply(flowing), Optional.<IExtendedBlockState>absent());
	}
	...
	private static final class BakedFluidTextured implements IPerspectiveAwareModel {
		private static final int x[] = { 0, 0, 1, 1 };
		private static final int z[] = { 0, 1, 1, 0 };
		private static final float eps = 1e-3F;

		private final LoadingCache<Long, BakedFluidTextured> modelCache = CacheBuilder.newBuilder().maximumSize(500).build(new CacheLoader<Long, BakedFluidTextured>() {
			@Override
	...
	public IModel retexture(ImmutableMap<String, String> textures) {
		...
		return new ModelFluidTextured(still, flowing);
	}
}